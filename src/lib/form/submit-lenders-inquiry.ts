...
export type SubmitLendersInquiryResult = SubmitContactInquiryResult;

function buildSubject(payload: LendersInquiryPayload): string {
  const interest = LENDERS_VERTICAL_INTEREST[payload.vertical];
  const interestLabel = getPartnersInterestLabel(interest);
  const subject = `Lenders partnership: ${payload.vertical} — ${interestLabel} — ${payload.company}`;
  return subject.length > 150 ? `${subject.slice(0, 147)}...` : subject;
}

function buildDescription(payload: LendersInquiryPayload): string {
  const lines = [
...
export async function submitLendersInquiry(
  payload: LendersInquiryPayload
): Promise<SubmitLendersInquiryResult> {
